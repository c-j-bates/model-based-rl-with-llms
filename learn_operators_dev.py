...
learn_operators_prompt = main_prompt_no_operators + \
"""
UTILS (Utility functions for import in interaction rule):
{utils}
...
GOAL: Attain condition for level_cleared predicate

INSTRUCTIONS:
Your job right now is to create code for your OPERATORS which are essentially functions that you can
use to reach the goal state which is specified by the level_cleared predicate. 
Think of how you would need to alter the current_state to satisfy the level_cleared conditions and what
functions you would need to write in order to transform the current_state to a state that satisfies the goal state
(as described by the level_cleared predicate)

It should look like this:

```python
# Goal
import1 #imports as needed
import2 

def operator1(args)  # Callable with exec
...
Think about these operators in terms of the types of actions you can perform and how they would change the state/environment.
...
# Do we actually need the predicates here?
infer_operator_prompt = main_prompt + \
"""
# OPERATORS
# (High-level operators for constructing plans)
{operators}
...
INSTRUCTIONS:
Your job right now is to create a piece of code that serves as part of your world model.
You should rewrite any OPERATOR that did not have its forward function consistent with
the OBSERVATIONS.
...
def load_and_extract_base_class(file_path, class_name):
    with open(file_path, 'r') as file:
        file_contents = file.read()
    
    # Regular expression to extract the class definition
    class_pattern = rf'class {class_name}\b[\s\S]*?(?=^class |\Z)'
    match = re.search(class_pattern, file_contents, re.MULTILINE)
...
class TBRLAgent:
    ...
    def __init__(
        ...
        do_revise_operators=False,
        do_learn_operators_scratch=True,
        ...
        self.do_revise_operators = do_revise_operators
        self.do_learn_operators_scratch = do_learn_operators_scratch
        ...
        self.infer_operator_prompt = infer_operator_prompt
        ...
        self.learn_operators_prompt = learn_operators_prompt
    ...
    def _update_world_model(self, text, rule_keys, save_to_file=True):
        print("WORLD MODEL UPDATE STARTED")
        x = re.findall(r'```python([\s\S]*?)```', text)
        if not len(x):
        ...
        if x:
            ...
            try:
                ...
                print("Executed locals:", my_locals)  # Debug print to see what's inside my_locals after exec

            except Exception as e:
                print("THERE WAS AN EXCEPTION THAT FAILED WORLD MODEL")
                return e
            else:
                if not 'interaction_rules' in my_locals:
                    print("WHERE ARE INTERACTION RULES LIN LOCAL")
                    return 'Exception: Could not find interaction_rules'
                interaction_rules = my_locals['interaction_rules']
                if len(interaction_rules):
                    print("INTERACTION RULES HAS LENGTH")
                    err_msg = ''
                    ...
                    print("SAVING INTERACTION RULES NOW")
                    self._save_interaction_rules_to_file()
                    return None if not err_msg else err_msg
                else:
                    print('Exception: Could not find any interaction rules')
                    return 'Exception: Could not find any interaction rules'
        else:
            print('Exception: No code found inside Python tags.')
    ...
    def _revise_world_model(self):
        ...
        # self.tape[-1]['revision_responses'] = {}
         ...
         # Load and extract the base class definition from _model_tmp.py
        base_class_code = load_and_extract_base_class('_model_tmp.py', 'BaseInteractionRule')
        ...
        for key, rule in self.runtime_vars['interaction_rules'].items():
            ...
            if self._do_revise_model(error_count):
                ...
                # Prepend the base class definition and interaction_rules initialization to the response
                resp_with_base_class = f"```python\n{base_class_code}\n\ninteraction_rules = []\n{resp}\n```"
                resp_with_base_class = resp_with_base_class.replace('\n```python', '\n```\n```python')

                self.world_model_save_name = '_model_tmp_revised'
                print("REVISED NAME:", self.world_model_save_name)
                print("THIS IS THE KEY TO BE REVISED:", [key])
                # self._update_world_model(resp, [key])  
                self._update_world_model(resp_with_base_class, [key])
    ...
    def _revise_operators(self):
        """
        (Re-)synthesize operator given examples, including an observed
        transition and any prediction errors under current world model
        """
        if not self.do_revise_operators:
            return
        
        prompt = self._make_langchain_prompt(
            self.infer_operator_prompt,  # TODO
            **{
                'state_format': self.engine.state_format,
                'actions_set': self.engine.actions_set,
                'interaction_rules': '\n'.join(self.runtime_vars['interaction_rules_str'].values()),
        ...
        resp = self.query_lm(prompt)
        self.tape[-1]['operator_revision_prompts'][str(key)] = prompt.to_messages()[0].content
        self.tape[-1]['operator_revision_responses'][str(key)] = resp
        print("REVISE OPERATOR LLM RESONSE START")
        print(resp)
        print("REVISE OPERATOR LLM RESPONSE END")
        return resp

    def _learn_operators_scratch(self, state):
        """
        Learn operators from scratch from predicates, operator and world model 
        """
        prompt = self._make_langchain_prompt(
            self.learn_operators_prompt,
            **{
                'state_format': self.engine.state_format,
                'actions_set': self.engine.actions_set,
                'predicates': self.runtime_vars['predicates'].replace('{', '{{').replace('}', '}}'),
        ...
        # breakpoint()
        print("LEARN OPERATOR SCRATCH LLM RESONSE START")
        resp = self.query_lm(prompt)
        print(resp)
        print("LEARN OPERATOR SCRATCH LLM RESONSE START")
        breakpoint()
    ...
    def _save_interaction_rules_to_file(self):
        ...
        print(f"SAVING RULES TO FILE: {file_name}")

        # Write rules to file
        with Path(file_name).open('w') as fid:
            fid.write(rules)
    ...
    def run(self, engine, max_iters=1):
        ...
        if self.do_revise_operators:
            self.tape[-1]['operator_revision_prompts'] = {}
            self.tape[-1]['operator_revision_responses'] = {}
        ...
        for i in range(max_iters):
            ...
            if self.do_learn_operators_scratch:
                self.tape[-1]['operator_learn_prompts'] = {}
                self.tape[-1]['operator_learn_responses'] = {}
                self._learn_operators_scratch(self.runtime_vars['observations'][-1]) # learn operators through current observation
...
if __name__ == '__main__':
    ...
    parser.add_argument('--learn-operators', action='store_true')
    parser.add_argument('--learn-operators-scratch', action='store_true')
    ...
    for level_set, level_id in levels:
        ...
        agent = TBRLAgent(
            ...
            do_revise_operators=args.learn_operators,
            do_learn_operators_scratch=args.learn_operators_scratch
        )
        agent.run(engine)
    ...
    import time
    import json
    tape_path = f'tapes2/{args.game}_{level_set}_{level_id}_{time.time()}.json'
