(define (problem level4)
    ...
    (:init
        ...
        ; ; Mapping rules to object control
        ; (rule_formed baba_word is_word_1 you_word)
        ; (is-controllable baba_obj)

        ; ; mapping goal condition
        ; ; (not (rule_formed flag_word is_word_2 win_word))
        ; (not (rule_formed flag_word is_word_2 win_word))

        ; formed rules
        (rule_formed baba_word is_word_1 win_word_1)
        (rule_formed grass_word is_word_2 win_word_2)
        (rule_formed flag_word is_word_3 you_word_1)
        (rule_formed love_word is_word_4 you_word_2)

        (is-controllable love_obj)
        (is-controllable flag_obj)

         ;; Initial locations for words forming rules
        (at baba_word loc-1-8)
        (at grass_word loc-1-1)
        (at love_word loc-6-1)
        (at flag_word loc-6-8)

        (at baba_obj loc-2-6)
        (at grass_obj loc-2-3)
        (at flag_obj loc-7-6)
        (at love_obj loc-7-3)
    ...
    ; (:goal (and
    ;     ;; Ensure the rule "flag_word is_word win_word" is formed
    ;     (rule_formed flag_word is_word_2 win_word)
    ;     ;; Ensure baba_obj is at the location of flag_obj
    ;     (at baba_obj loc-7-4)
    ;     ;; Ensure baba_obj is not at the flag's location until the rule is formed
    ;     ; (and (not (at baba_obj loc-7-4)) (not (rule_formed flag_word is_word_2 win_word))
    ;     (forall (?l - location)
    ;         (imply (at baba_obj ?l) (rule_formed flag_word is_word_2 win_word))
    ;     )
    ; ))
    ...
    ;     ;; Ensure baba_obj is not at the flag's location until the rule is formed
    ;     (forall (?l - location)
    ;         (imply (and (at baba_obj ?l) (at flag_obj ?l)) (rule_formed flag_word is_word_2 win_word))
    ;     )
    ; ))
...
(:goal (and (at flag_obj loc-2-6)))

(:goal (and (at flag_obj loc-2-3)))

(:goal (and (at love_obj loc-2-6)))

(:goal (and (at love_obj loc-2-3)))
