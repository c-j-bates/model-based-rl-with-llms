(define (problem level3)
    ...
    (:init
        ...
        (is-controllable baba_obj)
        (not (rule_formed baba_word is_word_1 win_word))

        (valid_rule baba_word is_word_1 win_word)
    
        ; (not (overlapping baba_obj flag_obj))

         ;; Initial locations for words forming rules
        (at baba_word loc-4-7)
    ...
    (:goal (and
        ;; Ensure the rule "flag_word is_word win_word" is formed
        (rule_formed baba_word is_word_1 win_word)
        ; (rule_formed baba_word is_word_2 you_word)

        ;; Ensure baba_obj is at the location of flag_obj
        ;; Ensure baba_obj is not at the flag's location unless the rule is formed
        ; (not  
        ;     (not (rule_formed flag_word is_word_2 win_word))
        ; )
    ))
