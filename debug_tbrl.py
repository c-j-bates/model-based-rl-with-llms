...
class TBRLAgent:
    ...
    def _update_plan(self, text):
        ...
        if x:
            ...
            #plan temp is being executed here to see if there are errors
            try:
    ...
    def _get_relevant_rules(self, obs):
        ...
        if self.sparse_interactions:
            ...
            # if self.interaction_rule_base_class in self.runtime_vars['interaction_rules_str'].values():
            #     print("included")
            # else:
            #     print('NOT INCLUDED')
            # resp = self.query_lm(prompt)
...
# goal_state_str = "overlapping(state['baba_obj'][0], state['flag_obj'][0]) and check_rule(state, ['flag_word', 'is_word', 'win_word'])"

# actions = []

# # Operator 1: Move 'baba_obj' towards 'flag_obj'
# new_actions, new_state = move_to(state, 'baba_obj', 0, state['flag_obj'][0])
# actions.extend(new_actions)

# # Operator 2: Check if the goal state is reached
# if eval(goal_state_str):
#     actions.append("Goal achieved!")
# else:
#     actions.append("Goal not achieved.")
#                    ```"""
    ...
    def _load_world_model(self, world_model_load_name=None, operators_load_name=None, predicates_load_name=None):
        if world_model_load_name:
            ...
            # print("HERE ARE MY KEYS", keys)
            err_msg = self._update_world_model("```python\n" + text + "\n```", keys)
            # print("HERE IS MY ERROR MESSAGE:", err_msg)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--world-model-file-name', type=str, default=None)
    ...
    # print("HERE ARE MY LEVELS:", levels)
    # print("HERE IS MY LEVELS len:", levels[0][1])
    ...
    import time
    import json
    tape_path = f'tapes3/{args.game}_{level_set}_{level_id}_{time.time()}.json'
    Path(tape_path).parent.mkdir(parents=True, exist_ok=True)
    with open(tape_path, 'w') as f:
        # print("AGENT TAPE BELOW")
...
# dict_keys(['planner_err', 'relevant_rules_prompt', 'relevant_rules_response', 'action', 'observation', 'world_model'])
